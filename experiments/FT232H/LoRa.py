...
def getter(register_address):
    ...
    def decorator(func):
        def wrapper(self):
            return func(self, self.spi.transfer([register_address, 0])[1])
        return wrapper
    return decorator
...
def setter(register_address):
    ...
    def decorator(func):
        def wrapper(self, val):
            return self.spi.transfer([register_address | 0x80, func(self, val)])[1]
        return wrapper
    return decorator
...
class LoRa(object):
    ...
    def __init__(self, spi, verbose=True):
    ...
    def get_mode(self):
        ...
        :return:    New mode
        """
        self.mode = self.spi.transfer([REG.LORA.OP_MODE, 0])[1]
        return self.mode
    ...
    def set_mode(self, mode):
        ...
        self.mode = mode
        return self.spi.transfer([REG.LORA.OP_MODE | 0x80, mode])[1]
    ...
    def write_payload(self, payload):
        ...
        return self.spi.transfer([REG.LORA.FIFO | 0x80] + payload)[1:]
    ...
    def read_payload(self , nocheck = False):
        ...
        payload = self.spi.transfer([REG.LORA.FIFO] + [0] * rx_nb_bytes)[1:]
        return payload
    ...
    def get_freq(self):
        ...
        :rtype:     float
        """
        msb, mid, lsb = self.spi.transfer([REG.LORA.FR_MSB, 0, 0, 0])[1:]
    ...
    def set_freq(self, f):
        ...
        i -= mid * 256
        lsb = i
        return self.spi.transfer([REG.LORA.FR_MSB | 0x80, msb, mid, lsb])

    def get_pa_config(self, convert_dBm=False):
        v = self.spi.transfer([REG.LORA.PA_CONFIG, 0])[1]
        pa_select    = v >> 7
        max_power    = v >> 4 & 0b111
    ...
    def set_pa_config(self, pa_select=None, max_power=None, output_power=None):
        ...
        return self.spi.transfer([REG.LORA.PA_CONFIG | 0x80, val])[1]
    ...
    def get_ocp(self, convert_mA=False):
        v = self.spi.transfer([REG.LORA.OCP, 0])[1]
        ocp_on = v >> 5 & 0x01
        ocp_trim = v & 0b11111
        if convert_mA:
            if ocp_trim <= 15:
    ...
    def set_ocp_trim(self, I_mA):
        ...
        ocp_on = self.spi.transfer([REG.LORA.OCP, 0])[1] >> 5 & 0x01
        if I_mA <= 120:
            v = int(round((I_mA-45.)/5.))
        else:
            v = int(round((I_mA+30.)/10.))
        v = set_bit(v, 5, ocp_on)
        return self.spi.transfer([REG.LORA.OCP | 0x80, v])[1]

    def get_lna(self):
        v = self.spi.transfer([REG.LORA.LNA, 0])[1]
        return dict(
                lna_gain     = v >> 5,
    ...
    def set_lna(self, lna_gain=None, lna_boost_lf=None, lna_boost_hf=None):
        ...
        retval = self.spi.transfer([REG.LORA.LNA | 0x80, val])[1]
        if lna_gain is not None:
    ...
    def get_fifo_addr_ptr(self):
        return self.spi.transfer([REG.LORA.FIFO_ADDR_PTR, 0])[1]

    def set_fifo_addr_ptr(self, ptr):
        return self.spi.transfer([REG.LORA.FIFO_ADDR_PTR | 0x80, ptr])[1]

    def get_fifo_tx_base_addr(self):
        return self.spi.transfer([REG.LORA.FIFO_TX_BASE_ADDR, 0])[1]

    def set_fifo_tx_base_addr(self, ptr):
        return self.spi.transfer([REG.LORA.FIFO_TX_BASE_ADDR | 0x80, ptr])[1]

    def get_fifo_rx_base_addr(self):
        return self.spi.transfer([REG.LORA.FIFO_RX_BASE_ADDR, 0])[1]

    def set_fifo_rx_base_addr(self, ptr):
        return self.spi.transfer([REG.LORA.FIFO_RX_BASE_ADDR | 0x80, ptr])[1]

    def get_fifo_rx_current_addr(self):
        return self.spi.transfer([REG.LORA.FIFO_RX_CURR_ADDR, 0])[1]

    def get_fifo_rx_byte_addr(self):
        return self.spi.transfer([REG.LORA.FIFO_RX_BYTE_ADDR, 0])[1]

    def get_irq_flags_mask(self):
        v = self.spi.transfer([REG.LORA.IRQ_FLAGS_MASK, 0])[1]
        return dict(
                rx_timeout     = v >> 7 & 0x01,
                rx_done        = v >> 6 & 0x01,
                crc_error      = v >> 5 & 0x01,
    ...
    def set_irq_flags_mask(self,
        ...
        return self.spi.transfer([REG.LORA.IRQ_FLAGS_MASK | 0x80, v])[1]

    def get_irq_flags(self):
        v = self.spi.transfer([REG.LORA.IRQ_FLAGS, 0])[1]
        return dict(
                rx_timeout     = v >> 7 & 0x01,
                rx_done        = v >> 6 & 0x01,
                crc_error      = v >> 5 & 0x01,
    ...
    def set_irq_flags(self,
        ...
        for i, s in enumerate(['cad_detected', 'fhss_change_ch', 'cad_done', 'tx_done', 'valid_header',
            ...
            if this_bit is not None:
                v = set_bit(v, i, this_bit)
        return self.spi.transfer([REG.LORA.IRQ_FLAGS | 0x80, v])[1]

    def clear_irq_flags(self):
        v = self.spi.transfer([REG.LORA.IRQ_FLAGS | 0x80, 0])[1]
        return v

    def get_rx_nb_bytes(self):
        return self.spi.transfer([REG.LORA.RX_NB_BYTES, 0])[1]

    def get_rx_header_cnt(self):
        msb, lsb = self.spi.transfer([REG.LORA.RX_HEADER_CNT_MSB, 0, 0])[1:]
        return lsb + 256 * msb

    def get_rx_packet_cnt(self):
        msb, lsb = self.spi.transfer([REG.LORA.RX_PACKET_CNT_MSB, 0, 0])[1:]
        return lsb + 256 * msb

    def get_modem_status(self):
        status = self.spi.transfer([REG.LORA.MODEM_STAT, 0])[1]
        return dict(
    ...
    def get_pkt_snr_value(self):
        v = self.spi.transfer([REG.LORA.PKT_SNR_VALUE, 0])[1]
        return float(256-v) / 4.

    def get_pkt_rssi_value(self):
        v = self.spi.transfer([REG.LORA.PKT_RSSI_VALUE, 0])[1]
        return v - 157

    def get_rssi_value(self):
        v = self.spi.transfer([REG.LORA.RSSI_VALUE, 0])[1]
        return v - 157

    def get_hop_channel(self):
        v = self.spi.transfer([REG.LORA.HOP_CHANNEL, 0])[1]
        return dict(
                pll_timeout          = v >> 7,
    ...
    def get_modem_config_1(self):
        val = self.spi.transfer([REG.LORA.MODEM_CONFIG_1, 0])[1]
        return dict(
                bw = val >> 4 & 0x0F,
    ...
    def set_modem_config_1(self, bw=None, coding_rate=None, implicit_header_mode=None):
        ...
        return self.spi.transfer([REG.LORA.MODEM_CONFIG_1 | 0x80, val])[1]
    ...
    def get_modem_config_2(self, include_symb_timout_lsb=False):
        val = self.spi.transfer([REG.LORA.MODEM_CONFIG_2, 0])[1]
        d = dict(
    ...
    def set_modem_config_2(self, spreading_factor=None, tx_cont_mode=None, rx_crc=None):
        ...
        return self.spi.transfer([REG.LORA.MODEM_CONFIG_2 | 0x80, val])[1]
    ...
    def get_modem_config_3(self):
        val = self.spi.transfer([REG.LORA.MODEM_CONFIG_3, 0])[1]
        return dict(
    ...
    def set_modem_config_3(self, low_data_rate_optim=None, agc_auto_on=None):
        ...
        return self.spi.transfer([REG.LORA.MODEM_CONFIG_3 | 0x80, val])[1]
    ...
    def get_symb_timeout(self):
        ...
        msb, lsb = self.spi.transfer([SYMB_TIMEOUT_MSB, 0, 0])[1:]    # the MSB bits are stored in REG.LORA.MODEM_CONFIG_2
        msb = msb & 0b11
        return lsb + 256 * msb

    def set_symb_timeout(self, timeout):
        bkup_reg_modem_config_2 = self.spi.transfer([REG.LORA.MODEM_CONFIG_2, 0])[1]
        ...
        old_msb = self.spi.transfer([REG.LORA.MODEM_CONFIG_2  | 0x80, reg_modem_config_2])[1] & 0x03
        old_lsb = self.spi.transfer([REG.LORA.SYMB_TIMEOUT_LSB | 0x80, lsb])[1]
        return old_lsb + 256 * old_msb

    def get_preamble(self):
        msb, lsb = self.spi.transfer([REG.LORA.PREAMBLE_MSB, 0, 0])[1:]
        return lsb + 256 * msb

    def set_preamble(self, preamble):
        msb = preamble >> 8
        lsb = preamble - msb * 256
        old_msb, old_lsb = self.spi.transfer([REG.LORA.PREAMBLE_MSB | 0x80, msb, lsb])[1:]
        return old_lsb + 256 * old_msb
    ...
    def get_fei(self):
        msb, mid, lsb = self.spi.transfer([REG.LORA.FEI_MSB, 0, 0, 0])[1:]
        msb &= 0x0F
    ...
    def get_register(self, register_address):
        return self.spi.transfer([register_address & 0x7F, 0])[1]

    def set_register(self, register_address, val):
        return self.spi.transfer([register_address | 0x80, val])[1]

    def get_all_registers(self):
        # read all registers
        reg = self.spi.transfer([1]+[0]*0x3E)[1:]
        reg = [0] + list(reg)
        self.mode = reg[1]
        return reg
